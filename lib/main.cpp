...
MYLIBAPI string ExecCmdEx(const char * ip_temp,const char * str_cmd);
MYLIBAPI void OpenTriggerFun(const char*ip);
    ...
    while(true)
    {
        ...
        switch(iSelect)
        {
            ...
            case 3:
            {
                ...
                string code_temp2 = "$C0$B6$CF$E6A23456";
                string m_szCameraIp = "192.168.28.100";
                string temp = "1123721327";
                string sUrl3 = "http://test.dev.365etc.com/java/api/"+code_temp2+"/"+m_szCameraIp+"/"+temp;
                                                            	char* str_java = (char*)malloc(BUFSIZE);
                                                                memset(str_java, 0, BUFSIZE);
                                                                if(Http->HttpGet(sUrl3.c_str(),str_java)) {
                                                                    printf("\n->%s\n\n", str_java);
                                                                }
                                                                free(str_java);
            }
            ...
            case 4:
            {
                //OpenTriggerFun("192.168.0.1");
                ExecCmdEx("192.168.0.1","SoftTriggerCapture");
            }
...
string ExecCmdEx(const char * ip_temp,const char * str_cmd)
{
    ip_temp = "192.168.28.100";
    printf("ExecCmdEx=>%s,%s Open...\n\n",ip_temp,str_cmd);

    HVAPI_HANDLE_EX hHandle = HVAPI_OpenEx(ip_temp,NULL);
    if(hHandle == NULL)
    {
            printf("ExecCmdEx %s Open Filed\n\n",ip_temp);
    }
    else
    {
       printf("ExecCmdEx %s Open Success\n\n",ip_temp);
       //HV_API_EX HRESULT CDECL HVAPI_ExecCmdEx(
       ...
       // INT* pnRetLen);
       INT* len;
       //LPTSTR lpStr = new TCHAR[1024];
       char chTimeFmt[100] = {0};
       HRESULT r = HVAPI_ExecCmdEx(hHandle,"SoftTriggerCapture",chTimeFmt,1024,len);
       if(r == S_OK)
       {
         printf("ExecCmdEx %s Open Ex Success\n\n",ip_temp);
       }
       else
       {
          printf("ExecCmdEx %s Open Ex Filed\n\n",ip_temp);
       }
       HVAPI_CloseEx(hHandle);
    }
    return "";
}
void OpenTriggerFun(const char * ip_temp)
{
        //const char * ip_temp = ip.c_str();
        //printf("OpenTriggerFun %s Open Filed\n\n",ip.c_str());
        printf("OpenTriggerFun GET %s\n", ip_temp);
        HVAPI_HANDLE_EX hHandle = HVAPI_OpenEx(ip_temp,NULL);
        if(hHandle == NULL)
        {
            printf("OpenTriggerFun %s Open Filed\n\n",ip_temp);
        ...
        {
            printf("OpenTriggerFun %s Open Success\n\n",ip_temp);
            HRESULT r = HVAPI_TriggerSignal(hHandle,0);
            if(r == S_OK)
            {
                printf("OpenTriggerFun %s Open Passageway Success\n\n",ip_temp);
            }
            else
            {
                printf("OpenTriggerFun %s Open Passageway Filed\n\n",ip_temp);
            }
            HVAPI_CloseEx(hHandle);
        }
}
...
string GetUrlIn(string sUrl)
{
    ...
    string sUrl1 = "http://test.dev.365etc.com/front/Testa/test/?sys=record&plateno="+sTemp+"&ip=192.168.10.100";
    return GetUrlHttp(sUrl1);
}
